...
const droneVelCtx = droneVelCanvas.getContext('2d');

const droneAccCanvas = document.getElementById('drone-acc-canvas');
const droneAccCtx = droneAccCanvas.getContext('2d');
...
function drawDroneMission() {
    ...
    droneObstacles.forEach(obs => {
        ...
        droneMapCtx.fillStyle = "rgba(239, 68, 68, 0.22)";
        droneMapCtx.strokeStyle = COLOR.danger;
        droneMapCtx.lineWidth = 1.5;
        droneMapCtx.fill();
        droneMapCtx.stroke();
        
        droneMapCtx.beginPath();
        droneMapCtx.moveTo(cCenter.x - 4, cCenter.y - 4);
        droneMapCtx.lineTo(cCenter.x + 4, cCenter.y + 4);
        droneMapCtx.moveTo(cCenter.x + 4, cCenter.y - 4);
        droneMapCtx.lineTo(cCenter.x - 4, cCenter.y + 4);
        droneMapCtx.strokeStyle = COLOR.danger;
        droneMapCtx.stroke();
...
function switchTab(tabId) {
    ...
    if (tabId === 'calculadora') {
        ...
        document.getElementById("tab-drone-btn").classList.add("active");
